...
def segment_cp3_fast(img: np.ndarray, gpu: bool) -> tuple[np.ndarray, float]:
    """cellpose3_legacy — CP3 nuclei model with the opt-in fast preset."""
    from cellpose3_legacy import models as models3
    model = models3.CellposeModel(gpu=gpu, pretrained_model="nuclei")
    with warnings.catch_warnings():
        warnings.filterwarnings(
        ...
        masks, _, _ = model.eval(
            img, channels=[0, 0],
            fast_mode=True, fast_diameter=17.0,
